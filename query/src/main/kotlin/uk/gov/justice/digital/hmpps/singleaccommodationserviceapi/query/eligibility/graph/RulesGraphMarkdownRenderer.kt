 ...
 * - [`RuleA`](../.....RuleA.kt): Description
 *
 *
 * **RulesetName2** (RuleSet)
 * - [`RuleB`](../.....RuleB.kt): Description
 * - FAIL: [`Updater`](../.....Updater.kt) - Set Upcoming
 *
...
object RulesGraphMarkdownRenderer {
  ...
  fun render(graphs: List<RulesGraph>): String = buildString {
    ...
    appendLine()
    appendLine("Diamond RuleSet nodes have a FAIL context updater.")
    appendLine()
    ...
    graphs.sortedBy { it.treeName }.forEach { graph ->
      ...
      graph.nodes.forEach { node ->
        appendLine("  ${node.id}${mermaidShape(node)}")
      }
      /*
       RulesetName1 -->|PASS| RulesetName2
       RulesetName1 -->|FAIL| notConfirmed
  ...
  private fun mermaidShape(node: GraphNode): String {
    val text = escape(label(node))
    val diamond = node.kind == GraphNodeKind.RULE_SET && node.contextUpdater != null
    return if (diamond) "{\"$text\"}" else "[\"$text\"]"
  }
    ...
    if (node.kind == GraphNodeKind.RULE_SET) {
      ...
      if (node.rules.isEmpty()) {
        ...
        node.rules.forEach { rule ->
          appendLine("- ${codeLink(rule.className, rule.sourcePath)}: ${rule.description}")
        }
      }
  ...
  private fun failLabel(updater: ContextUpdaterInfo): String = if (updater.name == "constant") {
    ...
    "${codeLink(updater.name, updater.sourcePath)} - ${updater.description}"
  }

  private fun codeLink(className: String, sourcePath: String?): String = if (sourcePath.isNullOrBlank()) {
    "`$className`"
  } else {
  ...
  private fun actionCell(result: ServiceResultSpec): String {
    val action = result.serviceStatus.proposedAction ?: return "-"
    return "${action.name} (${result.serviceStatus.service.name})"
  }
  ...
  private data class UpdaterCatalogueRow(
    ...
    val description: String,
    val usedBy: String,
    val outcomes: Map<String, ServiceResultSpec>,
    val anchor: String,
  )
}
